...
class SlitGroup:
    def __init__(
        ...
        **kwargs,
        ...
        self.ibkg_list = kwargs.pop('ibkg_list', None)
    ...
    def make_diff_image(self, exp=1, separate=False):
        ...
        ibkg_list: list or None
            The background indice for each exposure. If None, use all exposures
            except for the science exposure.
        ...
        if self.meta["diffs"]:
            if ibkg_list is None:
                ineg = ~self.unp[exp]
            else:
#                iexp = np.where(exp, self.unp.values)[0][0]
                ineg = ibkg_list[exp]
...
def drizzle_grating_group(
    ...
    **kwargs,
    ...
    exclude_dithers = kwargs.pop('exclude_dithers', None)
    ...
    for k in grating_keys:
        ...
        if exclude_dithers is not None:
            tab = tab[~np.isin(tab['exposure_index'],\
                              np.array(exclude_dithers))]
            print(tab)
        tabs.append(tab)
